    ...
    for i in range(N):
        for j in range(N):
            ...
            for idx1, idx2, idx3 in zip(
                src_pt_index[i][j], dst_pt_index[i][j], landmark_index[i][j]
            ):
                pan, tilt = (
                    x0[landmark_start_index + idx3 * 2],
                    x0[landmark_start_index + idx3 * 2 + 1],
                )  # estimated landmark pan, tilt
                x1, y1 = keypoints[i][idx1][0], keypoints[i][idx1][1]  # observation
                x2, y2 = keypoints[j][idx2][0], keypoints[j][idx2][1]
                proj_x1, proj_y1 = TransFunction.from_ray_to_image(
                    u, v, fl1, pan1, tilt1, pan, tilt
                )  # projection by pose and landmark
                proj_x2, proj_y2 = TransFunction.from_ray_to_image(
                    u, v, fl2, pan2, tilt2, pan, tilt
                )
...
def bundle_adjustment(
    ...
    (
        ...
        landmark_index,
        n_landmark,
    ) = build_matching_graph(images, image_match_mask, feature_method, verbose)
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            for idx1, idx2, idx3 in zip(
                ...
                landmark = TransFunction.from_image_to_ray(
                    u, v, ptz1[2], ptz1[0], ptz1[1], x1, y1
                )
                x0[
                    landmark_start_index + idx3 * 2 : landmark_start_index
                    + idx3 * 2
                    + 2
                ] = landmark
    n_pose = N
    ...
    for i in range(N):
        ...
        key_frame = KeyFrame(
            images[i], image_indices[i], center, rotation, u, v, pan, tilt, fl
        )
        ...
        if verbose:
            print(
                "frame %d, landmark number %d"
                % (image_indices[i], len(key_frame.landmark_index))
            )
...
def ut_build_adjustment_from_image_sequence():
    ...
    (
        ...
        landmark_index,
        n_landmark,
    ) = build_matching_graph(images, image_match_mask, True)
    ...
    for i in range(N):
        (
            ...
            tilt,
            fl,
        ) = pose_estimate[i * 3 : i * 3 + 3]
        keyframe = KeyFrame(
            images[i], image_index[i], cc, base_rotation, u, v, pan, tilt, fl
        )
...
def ut_bundle_adjustment_interface():
    ...
    landmarks, keyframes = bundle_adjustment(
        images,
        image_index,
        "orb",
        initial_ptzs,
