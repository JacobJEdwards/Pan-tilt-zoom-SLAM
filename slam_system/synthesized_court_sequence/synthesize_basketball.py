...
class DataSynthesize:
    ...
    def generate_points(num):
        ...
        for i in range(num):
            ...
            if choice < 3:
                ...
                list_pts.append(
                    [
                        x_side * random.gauss(0, 2)
                        + (1 - x_side) * random.gauss(29, 2),
                        random.uniform(0, 15),
                        random.uniform(0, 5),
                    ]
                )
                ...
                list_pts.append(
                    [random.uniform(0, 29), random.gauss(15, 2), random.uniform(0, 5)]
                )
            else:
    ...
    def show_image_sequence(self, pts):
        ...
        for i in range(self.annotation.size):
            ...
            for j in range(len(pts)):
                ...
                res = TransFunction.from_3d_to_2d(
                    self.u,
                    self.v,
                    f,
                    pan,
                ...
                res2 = TransFunction.from_pan_tilt_to_2d(
    ...
    def draw_3d_model(self, features):
        ...
        for i in range(len(self.line_index)):
            x = [
                self.points[int(self.line_index[i][0])][0],
                self.points[int(self.line_index[i][1])][0],
            ]
            y = [
                self.points[int(self.line_index[i][0])][1],
                self.points[int(self.line_index[i][1])][1],
            ]
            z = [0, 0]
            ax.plot(x, y, z, color="g")

        ax.scatter(
            features[:, 0], features[:, 1], features[:, 2], color="r", marker="o"
        )
        plt.show()
    ...
    def computer_all_ray(self, points):
        ...
        for i in range(0, len(points)):
            ray = TransFunction.compute_rays(
                self.proj_center, pts[i], self.base_rotation
            )
            all_rays.append(ray)
        return all_rays
