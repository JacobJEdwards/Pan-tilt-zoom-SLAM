...
class Map:
    ...
    def add_keyframe_with_ba(self, keyframe, save_path, verbose=False):
        ...
        for i in range(len(keyframes)):
            ...
            if keyframe.get_feature_num() > 0:
                ...
                print(
                    "warning: key frame, %d, image index %d is not included in the map"
                    % (i, image_indices[i])
                )

        if verbose:
            print(
                "updated map, number of key frame: %d, number of landmark %d"
    ...
    def good_new_keyframe(
        self, ptz, threshold1=5, threshold2=20, im_width=1280, verbose=False
    ):
        """
        ...
        for i in range(N):
            pan_angle_overlaps[i] = overlap_pan_angle(
                ptz[2], ptz[0], map_ptzs[i][2], map_ptzs[i][0], im_width
            )
...
class RandomForestMap:
    def __init__(self):
        self.keyframe_location = (
            "C:/graduate_design/Pan-tilt-zoom-SLAM/slam_system/random_forest/keyframes/"
        )
...
def ut_add_first_key_frame():
    ...
    keyframe = KeyFrame(
        im, image_index[0], camera_center, base_rotation, u, v, ptz[0], ptz[1], ptz[2]
    )
...
def ut_good_new_keyframe():
    ...
    for i in range(0, 3600, 1):
        ptz = input.get_ptz(i)
        keyframe = KeyFrame(
            im, i, camera_center, base_rotation, u, v, ptz[0], ptz[1], ptz[2]
        )
